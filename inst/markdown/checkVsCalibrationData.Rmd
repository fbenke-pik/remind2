...
scen <- gdx2::readGDX(params$gdx, "c_expname")[[1]]

vars <- setdiff(
  c(
    gdx2::readGDX(params$gdx, "ppf", restoreZeros = FALSE),
    gdx2::readGDX(params$gdx, "industry_ue_calibration_target_dyn37", restoreZeros = FALSE)
  ), "lab"
)

# baseline run quantities
q.baseline <- gdx2::readGDX(params$gdx, "vm_cesIO", select = list("_field" = "level"), restoreZeros = FALSE)

# calibration data quantities
q.calibration <- gdx2::readGDX(params$gdx, "pm_cesdata", select = list("cesParameter" = "quantity"), restoreZeros = FALSE)
...
p.baseline <- gdx2::readGDX(params$gdx, "o01_CESderivatives", 
...
p.calibration <- gdx2::readGDX(params$gdx, "pm_cesdata", restoreZeros = FALSE, select = list("cesParameter" = "price"))
...
deviations <- left_join(
  as.quitte(q.baseline) %>% 
    select("region", "variable" = "all_in", "period", "value") %>% 
    mutate(model = "Baseline"),
  as.quitte(q.calibration) %>% 
    select("region", "variable" = "all_in", "period", "value") %>% 
    mutate(model = "Calibration"),
  by = c("region", "period", "variable"), 
  suffix = c(".baseline", ".calibration")
) %>%
