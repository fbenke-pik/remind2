...
for (i in seq(length(params$gdxPaths))) {
  tmp <- gdx2::readGDX(
    params$gdxPaths[[i]], "pm_cesdata", 
    select = list("cesParameter" = "quantity"), restoreZeros = FALSE, 
    uniqueStyle = "classic")
  
  if (!is.null(params$gdxPathNames)) {
    tmp <- magclass::add_dimension(tmp,
...
data <- as.quitte(data) %>%
  select("region", "scenario", "variable" = "all_in", "period", "value")
