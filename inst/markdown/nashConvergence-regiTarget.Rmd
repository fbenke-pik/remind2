...
regi <- gdx2::readGDX(params$gdx, "regi")

ext_regi <- gdx2::readGDX(params$gdx, "ext_regi")

regi_groupExt <- gdx2::readGDX(params$gdx, "regi_groupExt")

regi_group <- gdx2::readGDX(params$gdx, "regi_group")

emiMktGroup <- gdx2::readGDX(params$gdx, "emiMktGroup")

tgts <-
  pm_emiMktTarget %>%
  select("ext_regi", "ttot2", "emiMktExt") %>%
  mutate_at(c("ttot2"), as.numeric) %>%
  mutate(group = paste0(ttot2, "_", emiMktExt))
...
regiEmiMktRescaleType <-
  gdx2::readGDX(params$gdx, "regiEmiMktRescaleType")
      ...
      p <- plot_ly(data = d, height = 350) %>% #
        ...
        layout(
          ...
          yaxis = list(
            ...
            zerolinewidth = 3,
            title = list(text = "Emissions (Mt CO2)", font = list(
              color =
                qttyPriceColor["qtty"]
            )),
            range = c(min(
          ...
          yaxis2 = list(
            ...
            zerolinewidth = 3,
            showgrid = FALSE,
            title = list(text = "Price (US$2005/t CO2)", font = list(
              color =
...
for (reg in tgtRegions) {
  ...
  for (subGroup in tgts %>%
    filter(ext_regi == reg) %>%
    pull(group)) {
    tgtPeriod <- strsplit(subGroup, "_", fixed = TRUE)[[1]][1]
    unit <- pm_emiMktTarget %>%
      filter(ext_regi == reg, ttot2 == tgtPeriod) %>%
      pull(emi_type_47)
