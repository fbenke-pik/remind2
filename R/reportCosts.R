...
reportCosts <- function(gdx,
  ...
  # Mappings
  ccs2te <- gdx2::readGDX(gdx, "ccs2te", uniqueStyle = "classic")
  enty2rlf <- gdx2::readGDX(gdx, c("pe2rlf", "enty2rlf"), format = "first_found", stringsAsFactors = FALSE)
  ...
  # Sets
  tenoccs <- gdx2::readGDX(gdx, c("teNoCCS", "tenoccs"), format = "first_found")
  temapall <- gdx2::readGDX(gdx, c("en2en", "temapall"), format = "first_found", uniqueStyle = "classic", stringsAsFactors = FALSE)
  tenotransform <- gdx2::readGDX(gdx, c("teNoTransform", "tenotransform"), format = "first_found")
  teNoTransform33 <- gdx2::readGDX(gdx, "teNoTransform33", react = "silent")
  tenotransform <- setdiff(tenotransform, teNoTransform33)
  stor <- gdx2::readGDX(gdx, c("teStor", "stor"), format = "first_found")
  grid <- gdx2::readGDX(gdx, c("teGrid", "grid"), format = "first_found")
  trade_pe <- gdx2::readGDX(gdx, c("tradePe", "trade_pe"), format = "first_found")
  trade_se <- gdx2::readGDX(gdx, "tradeSe")
  perenew <- gdx2::readGDX(gdx, c("peRe", "perenew"), format = "first_found")
  petyf <- gdx2::readGDX(gdx, c("peFos", "petyf"), format = "first_found")
  sety <- gdx2::readGDX(gdx, c("entySe", "sety"), format = "first_found")
  fety <- gdx2::readGDX(gdx, c("entyFe", "fety"), format = "first_found")
  potentialseLiq <- c("seliqbio", "seliqfos", "seliqsyn", "seliq", "sepet", "sedie")
  se_Liq <- intersect(potentialseLiq, sety)
  teccs <- gdx2::readGDX(gdx, c("teCCS", "teccs"), format = "first_found")
  pebio <- gdx2::readGDX(gdx, c("peBio", "pebio"), format = "first_found")
  emiMacMagpie <- gdx2::readGDX(gdx, "emiMacMagpie")
  emiMacMagpieN2O <- gdx2::readGDX(gdx, "emiMacMagpieN2O")
  emiMacMagpieCH4 <- gdx2::readGDX(gdx, "emiMacMagpieCH4")
  emiMacMagpieCO2 <- gdx2::readGDX(gdx, "emiMacMagpieCO2")

  # Parameters
  pm_conv_TWa_EJ <- 31.536
  sm_tdptwyr2dpgj <- 31.71 # TerraDollar per TWyear to Dollar per
  pm_data <- gdx2::readGDX(gdx, name = "pm_data")
  pm_petradecost2_Mp_fin <- gdx2::readGDX(gdx, name = "pm_petradecost2_Mp_fin", react = "silent")
  if (is.null(pm_petradecost2_Mp_fin)) {
    pm_petradecost2_Mp_fin <- gdx2::readGDX(gdx, name = "pm_costsTradePeFinancial")
  ...
  v_costom <- gdx2::readGDX(gdx, name = c("v_costOM", "v_costom"), select = list("_field" = "level"), format = "first_found")
  ...
  vm_prodFe <- gdx2::readGDX(gdx, name = "vm_prodFe", select = list("_field" = "level"),
                             restoreZeros = FALSE, uniqueStyle = "classic")
  cost_emu <- gdx2::readGDX(gdx, name = "v30_pebiolc_costs", select = list("_field" = "level"))
  bio_cost_adjfac <- gdx2::readGDX(gdx, name = "v30_multcost", select = list("_field" = "level"))
  ...
  if (is.null(v33_FEdemand)) { # compatibility with the CDR module before the portfolio was added
    CDR_FEdemand <- gdx2::readGDX(gdx, name = "vm_otherFEdemand", select = list("_field" = "level"),
                                  restoreZeros = FALSE)
  }

  # Equations
  finenbal <- gdx2::readGDX(gdx, name = c("fe2ppfEn", "finenbal"), type = "Set", format = "first_found", stringsAsFactors = FALSE)
  ...
  balfinen.m <- gdx2::readGDX(gdx, name = c("qm_balFeForCesAndEs", "qm_balFeForCes",
                                            "q_balFeForCes", "q_balfinen"), type = "Equation",
                              select = list("_field" = "marginal"), format = "first_found",
                              react = "silent")[finenbal]

  if (is.null(balfinen.m)) {
    vm_demFeSector <- gdx2::readGDX(gdx, name = "vm_demFeSector", select = list("_field" = "level"),
                                    restoreZeros = FALSE, uniqueStyle = "classic")

    demFeIndst.m <- gdx2::readGDX(gdx, name = "q37_demFeIndst", type = "Equation",
                                  restoreZeros = FALSE, select = list("_field" = "marginal"))

    demFeBuild.m <- gdx2::readGDX(gdx, name = "q36_demFeBuild", type = "Equation",
                                  restoreZeros = FALSE, select = list("_field" = "marginal"))

    balFe.m <- gdx2::readGDX(gdx, name = c("q_balFe", "qm_balFe"), type = "Equation",
                             restoreZeros = FALSE, select = list("_field" = "marginal"),
                             format = "first_found", uniqueStyle = "classic")

    demFeTrans.m <- gdx2::readGDX(gdx, name = "q35_demFeTrans", type = "Equation",
                                  restoreZeros = FALSE, select = list("_field" = "marginal"))
    ...
    quant_fe_trans_emiMkt <- dimSums(dimSums(vm_demFeSector[, y1, "trans"], dim = c(3.1), na.rm = TRUE), dim = c(3.2), na.rm = TRUE)[, , getNames(price_fe_trans_emiMkt[, y1, ])] # quantity of final energy in transportation per emission market
    price_fe_trans <- dimSums(price_fe_trans_emiMkt * quant_fe_trans_emiMkt, dim = 3.2, na.rm = TRUE) / dimSums(quant_fe_trans_emiMkt, dim = 3, na.rm = TRUE) # price of final energy in transportation (US$2017/GJ)
    febalForUe.m <- price_fe_trans
  } else {
    febalForUe.m <- gdx2::readGDX(gdx, name = c("q_balFeForUe", "q_balFe", "q_febal"),
                                  type = "Equation", select = list("_field" = "marginal"),
  ...
  op_costs_part2_foss <- function(pe, se, te, vm_prodSe, i_pe2se, p_dataeta, vm_costfu_ex, vm_fuelex, sm_tdptwyr2dpgj, pm_conv_TWa_EJ) {
    ...
    out <- dimSums(vm_costfu_ex[, , pe] / dimSums(vm_fuelex[, , pe], dim = 3), dim = 3, na.rm = TRUE) * sm_tdptwyr2dpgj * # Average supply costs
      dimSums(vm_prodSe[sub_pe2se] / p_dataeta[, , sub_pe2se$all_te], dim = 3, na.rm = TRUE) * pm_conv_TWa_EJ
    return(out)
  }

  op_costs_part2_bio_nuc <- function(pe, se, te, vm_prodSe, i_pe2se, p_dataeta, pm_conv_TWa_EJ) {
    sub_pe2se <- i_pe2se[(pe2se$all_enty %in% pe) & (i_pe2se$all_enty1 %in% se) & (i_pe2se$all_te %in% te), ]
    out <- dimSums(vm_prodSe[sub_pe2se] / p_dataeta[, , sub_pe2se$all_te], dim = 3, na.rm = TRUE) * pm_conv_TWa_EJ
    return(out)
  }
  ...
  op_costs <- function(ei, eo, te, e2e = pe2se, teall2rlf = teall2rlf, vm_prodE = NULL, pm_data = pm_data, vm_cap = vm_cap, v_investcost = v_investcost) {
    ...
    vm_cap <- dimSums(vm_cap, dim = 3.2, na.rm = TRUE) # get rid of grades which are not used in calculations below because all other variables dont have them neither

    out <- dimSums( # sum OMF over technologies (fixed operation & maintenance costs)
      collapseNames(pm_data[, , "omf"])[, , e2e_allte] *
        (dimSums((v_investcost[, , e2e_allte] * vm_cap[, , e2e_allte])[teall2rlf], dim = 3.2, na.rm = TRUE) # sum over rlf
        )[, , e2e_allte],
      dim = 3, na.rm = TRUE
    )

    if (!is.null(vm_prodE) && !length(e2e_allte) == 0) { # if either SE or FE is produced, then variable O&M is added
      out <- out + dimSums(collapseNames(pm_data[, , "omv"])[, , sub_e2e$all_te] * dimSums(vm_prodE[sub_e2e], dim = c(3.1, 3.2), na.rm = TRUE),
        dim = 3, na.rm = TRUE
      )
    }
  ...
  cost_Mport <- function(pety, pm_petradecost2_Mp_fin, pm_pvp, budget.m, Mport) {
    out <- dimSums((pm_petradecost2_Mp_fin[, , pety] + (pm_pvp[, , pety] / (budget.m + 1.e-10))) * Mport[, , pety], dim = 3, na.rm = TRUE) * 1000
    return(out)
  }
  ...
  if (!is.null(getNames(costsMAC))) tmp <- mbind(tmp, setNames(dimSums(costsMAC[, , emiMacMagpie], dim = 3, na.rm = TRUE) * 1000, "Costs|Land Use|MAC-costs (billion US$2017/yr)"))
  if (!is.null(getNames(costsMAC))) tmp <- mbind(tmp, setNames(dimSums(costsMAC[, , emiMacMagpieN2O], dim = 3, na.rm = TRUE) * 1000, "Costs|Land Use|MAC-costs|N2O (billion US$2017/yr)"))
  if (!is.null(getNames(costsMAC))) tmp <- mbind(tmp, setNames(dimSums(costsMAC[, , emiMacMagpieCH4], dim = 3, na.rm = TRUE) * 1000, "Costs|Land Use|MAC-costs|CH4 (billion US$2017/yr)"))
  if (!is.null(getNames(costsMAC))) tmp <- mbind(tmp, setNames(dimSums(costsMAC[, , emiMacMagpieCO2], dim = 3, na.rm = TRUE) * 1000, "Costs|Land Use|MAC-costs|CO2 (billion US$2017/yr)"))
  ...
  regi_on_gdx <- unique(gdx2::readGDX(gdx, name = "regi2iso")[, 1])
  ...
  cost <- v_costfu * 1000 - setNames(output[regi_on_gdx, , "Res|Average Supply Costs|Coal ($/GJ)"] * Xport[, , "pecoal"] * pm_conv_TWa_EJ, NULL) -
    ...
    dimSums(pm_XPortsPrice[, , trade_se] * Xport[, , trade_se], dim = 3, na.rm = TRUE) * 1000 +
    dimSums(Mport[, , trade_pe] * pebal.m[, , trade_pe] / (budget.m + 1.e-10), dim = 3, na.rm = TRUE) * 1000 + # imports valued with domestic market price
    dimSums(pm_MPortsPrice[, , trade_se] * Mport[, , trade_se], dim = 3, na.rm = TRUE) * 1000
  ...
  .drop_GLOs <- function(x) {
    x[setdiff(getItems(x, dim = 1), c("GLO", names(regionSubsetList))), , ]
  }
  ...
  tmp <- mbind(tmp, setNames(
    cost1 + cost2 + cost3 + output[regi_on_gdx, , "Investment|Energy Supply (billion US$2017/yr)"],
