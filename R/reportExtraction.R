...
#' @export
#' @importFrom magclass mbind mselect new.magpie getYears<- getYears setNames getSets getSets<- as.magpie
#' @importFrom dplyr filter mutate
#' @importFrom tidyr extract

reportExtraction <- function(gdx, regionSubsetList = NULL, t = c(seq(2005, 2060, 5), seq(2070, 2110, 10), 2130, 2150)) {
  ####### conversion factors ##########
  TWa_2_EJ <- as.numeric(1 / gdx2::readGDX(gdx, c("sm_EJ_2_TWa", "sm_ej2twyr"),
    format = "first_found"
  ))
  ...
  ####### read in needed data #########
  ## sets
  petyex <- gdx2::readGDX(gdx, c("peEx", "petyex"), format = "first_found")
  enty2rlf <- gdx2::readGDX(gdx, c("pe2rlf", "enty2rlf"), format = "first_found")
  t <- as.numeric(gdx2::readGDX(gdx, "ttot"))
  t <- t[t >= 2005]
  ...
  calcAvgExtractionCosts <- function(i_pe, i_fuelex, i_fuelexcum, i_grades) {
    ...
    tmp_fuelex <- i_fuelex[which(getItems(i_fuelex, dim = 1) != "GLO"), , ]
    tmp_fuelexcum <- i_fuelexcum[which(getItems(i_fuelexcum, dim = 1) != "GLO"), , ]

    # Generate magpie object containing timestep periods
    tmp_ts <- new.magpie(getItems(tmp_fuelex, dim = 1), t)
    tmp_ts[, , ] <- ts
    ...
    o_aec <- mbind(o_aec, dimSums(o_aec, dim = 1) / length(getItems(tmp_fuelex, dim = 1)))
  ...
  calcAvgExtractionCostsGradeUranium <- function(i_fuelexcum, i_datarog, i_fuelex, TDpTWa_2_DpGJ, uranium_conv = NULL, igrade) {
    ...
    # and not at the global level
    tmp_fuelexcum <- i_fuelexcum[which(getItems(i_fuelexcum, dim = 1) != "GLO"), , ]
    tmp_fuelex <- i_fuelex[which(getItems(i_fuelex, dim = 1) != "GLO"), , igrade]
    ...
    # Compute and add global values (as a mean over regions)
    o_aec <- mbind(o_aec, dimSums(o_aec, dim = 1) / length(getItems(tmp_fuelexcum, dim = 1)))
  ...
  mylist <- lapply(levels(tmp6$variable), function(x) {
    suppressWarnings(calcCumulatedDiscount(
      data = tmp6 %>%
