...
#' @importFrom magclass mselect mselect<- collapseDim getItems getYears
...
reportEmi <- function(gdx, output = NULL, regionSubsetList = NULL,
  ...
  sm_tgn_2_pgc <- as.numeric(gdx2::readGDX(gdx, "sm_tgn_2_pgc")) # conversion of GtN of N2 to GtC Co2eq
  sm_tgch4_2_pgc <- as.numeric(gdx2::readGDX(gdx, "sm_tgch4_2_pgc")) # conversion of MtCH4 to GtC Co2eq
  MtN2_to_ktN2O <- 44 / 28 * 1000 # conversion from MtN to ktN2O

  # switches relevant for emissions reporting
  cm_multigasscen <- gdx2::readGDX(gdx, "cm_multigasscen")
  ...
  teFosCCS <- gdx2::readGDX(gdx, "teFosCCS")
  ...
  # total GHG emissions
  vm_co2eq <- gdx2::readGDX(gdx, "vm_co2eq", select = list("_field" = "level"), restoreZeros = FALSE)[, t, ]
  # total emissions by gas
  vm_emiAllMkt <- gdx2::readGDX(gdx, "vm_emiAllMkt", select = list("_field" = "level"),
                                restoreZeros = FALSE)[, t, ]

  # total energy emissions from pe2se and se2fe conversions
  vm_emiTeDetailMkt <- gdx2::readGDX(gdx, c("vm_emiTeDetailMkt", "v_emiTeDetailMkt"),
                                     select = list("_field" = "level"), format = "first_found",
                                     restoreZeros = FALSE, uniqueStyle = "classic")
  # total energy emissions in REMIND
  vm_emiTeMkt <- gdx2::readGDX(gdx, c("vm_emiTeMkt", "v_emiTeMkt"), format = "first_found",
                               select = list("_field" = "level"), restoreZeros = FALSE)[, t, ]
  # emissions from MAC curves (non-energy emissions)
  vm_emiMacSector <- gdx2::readGDX(gdx, "vm_emiMacSector", select = list("_field" = "level"),
                                   restoreZeros = FALSE)[, t, ]
  p_co2lucSub <- gdx2::readGDX(gdx, "p_co2lucSub", restoreZeros = TRUE, react = "silent")[, t, ]

  # F-Gases
  vm_emiFgas <- gdx2::readGDX(gdx, "vm_emiFgas", select = list("_field" = "level"),
                              restoreZeros = FALSE)[, t, ]
  ...
  v_emiEnFuelEx <- gdx2::readGDX(gdx, "v_emiEnFuelEx", select = list("_field" = "level"),
                                 restoreZeros = FALSE)

  ### for emissions of energy system technologies
  # emission factors of technologies
  pm_emifac <- gdx2::readGDX(gdx, "pm_emifac", restoreZeros = FALSE, uniqueStyle = "classic")[, t, ]
  # emissions factors of non-energy use
  pm_emifacNonEnergy <- gdx2::readGDX(gdx, "pm_emifacNonEnergy", restoreZeros = FALSE, react = "silent")[, t, ]
  # primary energy demand (pe2se emissions factors applied to)
  vm_demPE <- gdx2::readGDX(gdx, "vm_demPE", select = list("_field" = "level"), restoreZeros = FALSE)[, t, ]
  ...
  vm_prodSe <- gdx2::readGDX(gdx, "vm_prodSe", select = list("_field" = "level"),
                             restoreZeros = FALSE, uniqueStyle = "classic")
  ...
  vm_emiCdrTeDetail <- gdx2::readGDX(gdx, c("vm_emiCdrTeDetail", "v33_emi"), select = list("_field" = "level"), restoreZeros = FALSE, react = "silent")[, t, ]
  ...
  vm_co2capture <- gdx2::readGDX(gdx, c("vm_co2capture", "v_co2capture"),
                                 select = list("_field" = "level"), format = "first_found",
                                 restoreZeros = FALSE)[, t, ]
  ...
  vm_co2emi_cdrFE_beforeCapture <- gdx2::readGDX(gdx, c("vm_co2emi_cdrFE_beforeCapture", "v33_co2emi_non_atm_gas"), select = list("_field" = "level"), restoreZeros = FALSE, react = "silent")[, t, ]
  v33_co2emi_non_atm_calcination <- gdx2::readGDX(gdx, "v33_co2emi_non_atm_calcination", select = list("_field" = "level"), restoreZeros = FALSE, react = "silent")[, t, ]
  ...
  sm_capture_rate_cdrmodule <- gdx2::readGDX(gdx, c("sm_capture_rate_cdrmodule", "s33_capture_rate"), react = "silent")
  te_oae33 <- gdx2::readGDX(gdx, "te_oae33", react = "silent")
  ...
  vm_emiIndCCS <- gdx2::readGDX(gdx, "vm_emiIndCCS", select = list("_field" = "level"), restoreZeros = FALSE)[, t, ]
  getSets(vm_emiIndCCS)[3] <- "secInd37" # relabel subsector dimension to match with other parameters

  # CO2 released by CCU
  vm_co2CCUshort <- gdx2::readGDX(gdx, "vm_co2CCUshort", select = list("_field" = "level"), restoreZeros = FALSE)[, t, ]
  # variable to release captured CO2 when no CCU capacities are standing anymore vent captured CO2
  v_co2capturevalve <- gdx2::readGDX(gdx, "v_co2capturevalve", select = list("_field" = "level"), restoreZeros = FALSE)[, t, ]
  ...
  # in some cases there are more technologies and more grades. Thus, explicitly pick ccsinje*
  # techs and their only grade '1'
  max_geolStorage <- gdx2::readGDX(gdx, "vm_co2CCS", select = list("_field" = "upper"),
                                   restoreZeros = FALSE, uniqueStyle = "classic")
  ...
  v37_plasticsCarbon <- gdx2::readGDX(gdx, "v37_plasticsCarbon",
    select = list("_field" = "level"), temporal = 1, spatial = 2,
    restoreZeros = FALSE, react = "silent", uniqueStyle = "classic"
  )[, t, ]

  vm_emiNonFosNonIncineratedPlastics <- gdx2::readGDX(
    gdx, c("v37_emiNonFosNonIncineratedPlastics", "vm_emiNonFosNonIncineratedPlastics"),
  ...
  v37_emiNonPlasticWaste <- gdx2::readGDX(gdx, "v37_emiNonPlasticWaste",
    select = list("_field" = "level"),
  ...
  vm_incinerationEmi <- gdx2::readGDX(gdx, c("vm_incinerationEmi", "v37_incinerationEmi"),
    select = list("_field" = "level"), restoreZeros = FALSE, spatial = 2, react = "silent",
    uniqueStyle = "classic")[, t, ]

  vm_incinerationEmi <- magclass::matchDim(vm_incinerationEmi, v37_plasticsCarbon, fill = 0)

  vm_incinerationCCS <- gdx2::readGDX(gdx, "vm_incinerationCCS",
    select = list("_field" = "level"), restoreZeros = FALSE, spatial = 2, react = "silent",
    uniqueStyle = "classic")[, t, ]
  ...
  vm_nonIncineratedPlastics <- gdx2::readGDX(gdx, "vm_nonIncineratedPlastics",
    select = list("_field" = "level"), restoreZeros = FALSE,
    spatial = 2, react = "silent"
  )[, t, ]

  v37_plasticWaste <- gdx2::readGDX(gdx, "v37_plasticWaste",
    select = list("_field" = "level"), restoreZeros = FALSE,
    spatial = 2, react = "silent", uniqueStyle = "classic"
  )[, t, ]

  pm_incinerationRate <- gdx2::readGDX(gdx, "pm_incinerationRate",
    restoreZeros = FALSE, spatial = 2, react = "silent")[, t, ]
  ...
  v37_feedstocksCarbon <- gdx2::readGDX(gdx, "v37_feedstocksCarbon", select = list("_field" = "level"),
                                        restoreZeros = FALSE, spatial = 2, uniqueStyle = "classic")
  # read in share of non-plastics carbon that gets emitted
  cm_nonPlasticFeedstockEmiShare <- gdx2::readGDX(gdx, "cm_nonPlasticFeedstockEmiShare") %>%
    as.vector()
  # read in share of plastics in feedstocks
  s37_plasticsShare <- gdx2::readGDX(gdx, "s37_plasticsShare") %>%
    as.vector()
  ...
  if (is.null(extraData)) {
    ...
    regionHash <- digest::digest(sort(gdx2::readGDX(gdx, "all_regi")), "xxhash32")
    ...
    all_regi <- gdx2::readGDX(gdx, "all_regi")
  ...
  for (r in getItems(WasteShares, dim = 1)) {
  ...
  EmiIndSubSec <- mbind(
    ...
    setNames(
      out[, , "Emi|CO2|Energy|Demand|Industry|Waste (Mt CO2/yr)"]
      * SolidShareSubsec[getItems(out, dim = 1), , "steel"]
        / GtC_2_MtCO2,
      "sesowst.fesos.steel.ETS"
    ),
    setNames(
      out[, , "Emi|CO2|Energy|Demand|Industry|Waste (Mt CO2/yr)"]
      * SolidShareSubsec[getItems(out, dim = 1), , "cement"]
        / GtC_2_MtCO2,
      "sesowst.fesos.cement.ETS"
    ),
    setNames(
      out[, , "Emi|CO2|Energy|Demand|Industry|Waste (Mt CO2/yr)"]
      * SolidShareSubsec[getItems(out, dim = 1), , "chemicals"]
        / GtC_2_MtCO2,
      "sesowst.fesos.chemicals.ETS"
    ),
    setNames(
      out[, , "Emi|CO2|Energy|Demand|Industry|Waste (Mt CO2/yr)"]
      * SolidShareSubsec[getItems(out, dim = 1), , "otherInd"]
        / GtC_2_MtCO2,
  ...
  energyEmissions <- subsector_emissions %>%
    mutate(
      secInd37 = case_when(
  ...
  energyEmissions <- as.magpie(energyEmissions, spatial = 2, temporal = 1, datacol = 4)
  getSets(energyEmissions) <- getSets(out)
  out <- mbind(energyEmissions, out)

  # process emissions for cement subsector
  processEmissionsCement <- gdx2::readGDX(gdx, c("vm_emiIndBase", "vm_macBaseInd"),
    format = "first_found",
    select = list("_field" = "level"),
    restoreZeros = FALSE
  )[, , "co2cement_process.cement"] * GtC_2_MtCO2
  getSets(processEmissionsCement) <- getSets(out)
  getNames(processEmissionsCement) <- "Emi|CO2|pre-CCS|Industrial Processes|Cement (Mt CO2/yr)"

  out <- mbind(processEmissionsCement, out)
  ...
  intraRegionFactor <- new.magpie(getItems(bunkersEmi, dim = 1), getYears(bunkersEmi), fill = 0) #  is equal to 0 for non EU countries
  #  is equal to 35% of total bunkers in average from 2000-2020 for EU27 + UKI countries
  if ("EUR" %in% getItems(bunkersEmi, dim = 1)) {
  ...
  EmiMAC <- new.magpie(
    getItems(vm_emiMacSector, dim = 1),
  ...
  if ("DEU" %in% getItems(vm_co2eq, dim = 1)) {
    emi_autoprod_2015 <- 9.2819 # from CEDS in Mt CO2/yr

    out <- mbind(out, new.magpie(getItems(vm_co2eq, dim = 1), getYears(vm_co2eq),
  ...
  pm_emiLULUCF_GrassiShift <- gdx2::readGDX(gdx, c("pm_emiLULUCF_GrassiShift", "p47_LULUCFEmi_GrassiShift"), restoreZeros = TRUE, react = "silent")[getItems(out, dim = 1), getYears(out), ]
  ...
  if (module2realisation["carbonprice", ]$realization == "NDC") {
    p45_CO2eqwoLU_goal <- gdx2::readGDX(gdx, "p45_CO2eqwoLU_goal",
      restoreZeros = TRUE
    )[getItems(out, dim = 1), getYears(out), ]
  ...
  regs.wo.glob <- getItems(out, dim = 1)[!getItems(out, dim = 1) == "GLO"]
