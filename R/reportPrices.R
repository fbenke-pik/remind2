...
#' @importFrom magclass mbind getYears setNames dimExists new.magpie lowpass
...
reportPrices <- function(gdx, output = NULL, regionSubsetList = NULL,
  ...
  pm_SEPrice <- gdx2::readGDX(gdx, "pm_SEPrice")
  pm_PEPrice <- gdx2::readGDX(gdx, c("p_PEPrice", "pm_PEPrice"), format = "first_found")

  vm_demFeSector <- gdx2::readGDX(gdx, "vm_demFeSector", select = list("_field" = "level"),
                                  restoreZeros = FALSE, uniqueStyle = "classic")[, t, ]
  prodSe <- gdx2::readGDX(gdx, "vm_prodSe", select = list("_field" = "level"),
                          restoreZeros = FALSE, uniqueStyle = "classic")[, t, ]
  seAgg <- gdx2::readGDX(gdx, name = "seAgg", type = "Set", stringsAsFactors = FALSE, react = "silent")
  seAgg2se <- gdx2::readGDX(gdx, name = "seAgg2se", type = "Set", stringsAsFactors = FALSE,
                            react = "silent", uniqueStyle = "classic")

  # subset price parameters to those entries used in the model
  pe2se <- gdx2::readGDX(gdx, "pe2se", stringsAsFactors = FALSE, uniqueStyle = "classic")
  se2fe <- gdx2::readGDX(gdx, "se2fe", stringsAsFactors = FALSE, uniqueStyle = "classic")
  sector2emiMkt <- gdx2::readGDX(gdx, "sector2emimkt", stringsAsFactors = FALSE, uniqueStyle = "classic")
  entyFe2Sector <- gdx2::readGDX(gdx, "entyFe2Sector", stringsAsFactors = FALSE, uniqueStyle = "classic")
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "feelt", emi_sectors = "trans"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
    ...
    setNames(
      dimSums(p_weights_FEprice_diepet * dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = c("fepet", "fedie"), emi_sectors = "trans"), dim = 3.3, na.rm = TRUE), dim = 3.1, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Transport|Liquids (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "feh2t", emi_sectors = "trans"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
  ...
  if (module2realisation["transport", 2] == "edge_esm") {
    out <- mbind(
      out,
      setNames(
        dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fegat", emi_sectors = "trans"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fedie", emi_sectors = "trans"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Transport|Liquids|HDV (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fepet", emi_sectors = "trans"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "feels", emi_sectors = "build"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Buildings|Electricity (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fegas", emi_sectors = "build"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Buildings|Gases (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "feh2s", emi_sectors = "build"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Buildings|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fehos", emi_sectors = "build"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Buildings|Liquids (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fehes", emi_sectors = "build"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Buildings|Heat (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fesos", emi_sectors = "build"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "feels", emi_sectors = "indst"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Industry|Electricity (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fegas", emi_sectors = "indst"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Industry|Gases (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "feh2s", emi_sectors = "indst"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Industry|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fehos", emi_sectors = "indst"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Industry|Liquids (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fehes", emi_sectors = "indst"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
      "Price|Final Energy|Industry|Heat (US$2017/GJ)"
    ),
    setNames(
      dimSums(mselect(p_weights_FEprice_mkt * pm_FEPrice, all_enty1 = "fesos", emi_sectors = "indst"), dim = 3.3, na.rm = TRUE) * tdptwyr2dpgj,
  ...
  out <- mbind(
    out,
    setNames(
      mselect(pm_SEPrice, all_enty = "seliqfos") * tdptwyr2dpgj,
      "Price|Secondary Energy|Liquids|Fossil (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "seliqbio") * tdptwyr2dpgj,
      "Price|Secondary Energy|Liquids|Biomass (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "seliqsyn") * tdptwyr2dpgj,
      "Price|Secondary Energy|Liquids|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "sesobio") * tdptwyr2dpgj,
      "Price|Secondary Energy|Solids|Biomass (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "sesofos") * tdptwyr2dpgj,
      "Price|Secondary Energy|Solids|Fossil (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "seel") * tdptwyr2dpgj,
      "Price|Secondary Energy|Electricity (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "seh2") * tdptwyr2dpgj,
      "Price|Secondary Energy|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "segabio") * tdptwyr2dpgj,
      "Price|Secondary Energy|Gases|Biomass (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "segafos") * tdptwyr2dpgj,
      "Price|Secondary Energy|Gases|Fossil (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "segasyn") * tdptwyr2dpgj,
      "Price|Secondary Energy|Gases|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_SEPrice, all_enty = "sehe") * tdptwyr2dpgj,
  ...
  if (!is.null(seAgg)) {
    for (i in seAgg) { # calculate weights based on SE production shares, for all possible secondary energy aggregations (set seAgg in REMIND code)
      weightsSe <- mbind(
        weightsSe,
        dimSums(mselect(prodSe, all_enty1 = unique(filter(seAgg2se, .data$all_enty == i)[, "all_enty1"])),
                dim = c(3.1, 3.3), na.rm = TRUE) /
    ...
    out <- mbind(
      out,
      setNames(
        dimSums(mselect(weightsSe * pm_SEPrice[, , getItems(weightsSe, dim = 3)], all_enty1 = unique(filter(seAgg2se, .data$all_enty == "all_seliq")[, "all_enty1"]))) * tdptwyr2dpgj,
        "Price|Secondary Energy|Liquids (US$2017/GJ)"
      ),
      setNames(
        dimSums(mselect(weightsSe * pm_SEPrice[, , getItems(weightsSe, dim = 3)], all_enty1 = unique(filter(seAgg2se, .data$all_enty == "all_seso")[, "all_enty1"]))) * tdptwyr2dpgj,
        "Price|Secondary Energy|Solids (US$2017/GJ)"
      ),
      setNames(
        dimSums(mselect(weightsSe * pm_SEPrice[, , getItems(weightsSe, dim = 3)], all_enty1 = unique(filter(seAgg2se, .data$all_enty == "all_sega")[, "all_enty1"]))) * tdptwyr2dpgj,
  ...
  pm_FEPrice_by_SE_Sector_EmiMkt <- gdx2::readGDX(gdx, c("pm_FEPrice_by_SE_Sector_EmiMkt", "p_FEPrice_by_SE_Sector_EmiMkt"), format = "first_found", react = "silent")
  pm_FEPrice_by_Sector_EmiMkt <- gdx2::readGDX(gdx, c("pm_FEPrice_by_Sector_EmiMkt", "p_FEPrice_by_Sector_EmiMkt"), format = "first_found", react = "silent")
  pm_FEPrice_by_SE_Sector <- gdx2::readGDX(gdx, c("pm_FEPrice_by_SE_Sector", "p_FEPrice_by_SE_Sector"), format = "first_found", react = "silent")
  pm_FEPrice_by_SE_EmiMkt <- gdx2::readGDX(gdx, c("pm_FEPrice_by_SE_EmiMkt", "p_FEPrice_by_SE_EmiMkt"), format = "first_found", react = "silent")
  pm_FEPrice_by_SE <- gdx2::readGDX(gdx, c("pm_FEPrice_by_SE", "p_FEPrice_by_SE"), format = "first_found", react = "silent")
  pm_FEPrice_by_Sector <- gdx2::readGDX(gdx, c("pm_FEPrice_by_Sector", "p_FEPrice_by_Sector"), format = "first_found", react = "silent")
  pm_FEPrice_by_EmiMkt <- gdx2::readGDX(gdx, c("pm_FEPrice_by_EmiMkt", "p_FEPrice_by_EmiMkt"), format = "first_found", react = "silent")
  pm_FEPrice_by_FE <- gdx2::readGDX(gdx, c("pm_FEPrice_by_FE", "p_FEPrice_by_FE"), format = "first_found", react = "silent")
  ...
  if (length(pm_FEPrice_by_FE) > 0) {
    ...
    for (i in seq_len(nrow(se.fe.sector.emiMkt))) {
  ...
  out <- mbind(
    out,
    setNames(
      mselect(pm_MPortsPrice, all_enty = "seel") * tdptwyr2dpgj,
      "Trade|Price|Imports|SE|Electricity (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_MPortsPrice, all_enty = "seh2") * tdptwyr2dpgj,
      "Trade|Price|Imports|SE|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_MPortsPrice, all_enty = "sesobio") * tdptwyr2dpgj,
      "Trade|Price|Imports|SE|Liquids|Biomass (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_MPortsPrice, all_enty = "seliqsyn") * tdptwyr2dpgj,
      "Trade|Price|Imports|SE|Liquids|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_MPortsPrice, all_enty = "segasyn") * tdptwyr2dpgj,
  ...
  out <- mbind(
    out,
    setNames(
      mselect(pm_XPortsPrice, all_enty = "seel") * tdptwyr2dpgj,
      "Trade|Price|Exports|SE|Electricity (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_XPortsPrice, all_enty = "seh2") * tdptwyr2dpgj,
      "Trade|Price|Exports|SE|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_XPortsPrice, all_enty = "sesobio") * tdptwyr2dpgj,
      "Trade|Price|Exports|SE|Liquids|Biomass (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_XPortsPrice, all_enty = "seliqsyn") * tdptwyr2dpgj,
      "Trade|Price|Exports|SE|Liquids|Hydrogen (US$2017/GJ)"
    ),
    setNames(
      mselect(pm_XPortsPrice, all_enty = "segasyn") * tdptwyr2dpgj,
  ...
  vm_costTeCapital <- gdx2::readGDX(gdx, "vm_costTeCapital", select = list("_field" = "level"), restoreZeros = FALSE)[, YearsFrom2005, tech] # [tr USD2017/TWh]
  p_teAnnuity <- gdx2::readGDX(gdx, c("p_teAnnuity", "pm_teAnnuity"), restoreZeros = FALSE, format = "first_found")[, , tech]
  vm_capFac <- gdx2::readGDX(gdx, "vm_capFac", select = list("_field" = "level"), restoreZeros = FALSE)[, YearsFrom2005, tech] * 8760
  pm_data_omf <- gdx2::readGDX(gdx, "pm_data", restoreZeros = FALSE)[, , "omf"][, , tech]
  ...
  pm_emifac <- gdx2::readGDX(gdx, "pm_emifac", restoreZeros = FALSE, uniqueStyle = "classic")[, YearsFrom2005, "co2"][, , tech.fossil] # [GtC CO2/TWa]
  pm_emifac <- pm_emifac * 1e9 / s_twa2mwh / 3.6 # [GtC CO2/TWa] -> [tC CO2/GJ]

  p_priceCO2 <- gdx2::readGDX(gdx, name = c("p_priceCO2", "pm_priceCO2"), format = "first_found", restoreZeros = FALSE) # [USD2017/tC CO2]
  if (length(p_priceCO2) > 0) {
    p_priceCO2 <- add_columns(p_priceCO2, addnm = setdiff(YearsFrom2005, getYears(p_priceCO2)), dim = 2, fill = NA)
    p_priceCO2 <- add_columns(p_priceCO2, addnm = setdiff(getItems(pm_emifac, dim = 1), getItems(p_priceCO2, dim = 1)), dim = 1, fill = NA)
  ...
  pm_tau_fe_tax <- gdx2::readGDX(gdx, c("p21_tau_fe_tax", "pm_tau_fe_tax"), format = "first_found")[, YearsFrom2005, entyFe2Sector] # [tr USD2017/TWa]
  pm_tau_fe_sub <- gdx2::readGDX(gdx, c("p21_tau_fe_sub", "pm_tau_fe_sub"), format = "first_found")[, YearsFrom2005, entyFe2Sector] # [tr USD2017/TWa]
  ...
  pm_SEPrice <- gdx2::readGDX(gdx, "pm_SEPrice")[, YearsFrom2005, se] / s_twa2mwh / 3.6 * 1e12 # [tr USD2017/TWa] -> [USD2017/GJ]
  pm_eta_conv <- gdx2::readGDX(gdx, "pm_eta_conv", restoreZeros = FALSE)[, YearsFrom2005, tech]
  ...
  ## save Rawdata
  out.rawdata <- out
  getNames(out.rawdata) <- paste0(
    unitsplit(getNames(out.rawdata))$variable, "|Rawdata (",
  ...
  if (!grepl("SSP2", gdx2::readGDX(gdx, c("cm_GDPpopScen", "cm_GDPscen"), format = "first_found")) &&
  ...
  getNames(out.lowpass) <- paste0(
    unitsplit(getNames(out.lowpass))$variable, "|Moving Avg (",
  ...
  out2 <- new.magpie(getItems(out, dim = 1), getYears(vm_demFeSector), getNames(out), fill = NA)
  ...
  if (all(p21_CO2TaxSectorMarkup == 0)) { # then all are identical
    ...
    for (pcname in c(
      "Price|Carbon|Demand|Buildings (US$2017/t CO2)", "Price|Carbon|Demand|Transport (US$2017/t CO2)",
    ...
    out <- mbind(
      out,
      setNames(
        (1 + p21_CO2TaxSectorMarkup[, , "build"]) * pm_taxCO2eqSum * 1000 * 12 / 44,
        "Price|Carbon|Demand|Buildings (US$2017/t CO2)"
      ),
      setNames(
        (1 + p21_CO2TaxSectorMarkup[, , "trans"]) * pm_taxCO2eqSum * 1000 * 12 / 44,
    ...
    pm_taxCO2eq_FE <- collapseNames(pm_taxCO2eqSum * (1 +
      (
        p21_CO2TaxSectorMarkup[, , "build"] * output_wo_GLO[getItems(p21_CO2TaxSectorMarkup, dim = 1), , "FE|Buildings (EJ/yr)"]
          + p21_CO2TaxSectorMarkup[, , "trans"] * output_wo_GLO[getItems(p21_CO2TaxSectorMarkup, dim = 1), , "FE|Transport (EJ/yr)"]
      ) / output_wo_GLO[getItems(p21_CO2TaxSectorMarkup, dim = 1), , "FE (EJ/yr)"]
    ))
    pm_taxCO2eq_Emi <- collapseNames(pm_taxCO2eqSum * (1 +
      (
        p21_CO2TaxSectorMarkup[, , "build"] * output_wo_GLO[getItems(p21_CO2TaxSectorMarkup, dim = 1), , "Emi|GHG|Gross|Energy|Demand|Buildings (Mt CO2eq/yr)"]
          + p21_CO2TaxSectorMarkup[, , "trans"] * output_wo_GLO[getItems(p21_CO2TaxSectorMarkup, dim = 1), , "Emi|GHG|Gross|Energy|Demand|Transport (Mt CO2eq/yr)"]
      ) / output_wo_GLO[getItems(p21_CO2TaxSectorMarkup, dim = 1), , "Emi|GHG|Gross|Energy (Mt CO2eq/yr)"]
    ))

    out <- mbind(out, setNames(
      pm_taxCO2eq_FE * 1000 * 12 / 44,
      "Price|Carbon (US$2017/t CO2)"
    )) # AggregatedbyFE
    out <- mbind(out, setNames(
      pm_taxCO2eq_Emi * 1000 * 12 / 44,
      "Price|Carbon|AggregatedByGrossCO2 (US$2017/t CO2)"
    )) # AggregatedByEmiGHGGross
  }

  peFos <- gdx2::readGDX(gdx, "peFos") # fossil PE carriers
  p21_tau_Import <- gdx2::readGDX(gdx, name = "p21_tau_Import", react = "silent")[, t, peFos]
  tax_import_type_21 <- gdx2::readGDX(gdx, name = "tax_import_type_21", react = "silent")
  ...
  #
  CaptureBal_tmp <- new.magpie(getItems(out, dim = 1), getYears(out), fill = NA)
  CaptureBal_tmp[, getYears(balcapture.m), ] <- balcapture.m
  out <- mbind(out, setNames(
    CaptureBal_tmp / (budget.m + 1e-10) / 3.66 * 1e3,
  ...
  if (is.null(regionSubsetList$EUR)) {
    ...
    co2EUprice[getItems(pm_taxCO2eq, dim = 1)[!getItems(pm_taxCO2eq, dim = 1) %in% regionSubsetList$EUR], , ] <- 0
  ...
  if (!is.null(pm_taxemiMkt)) {
    ...
    if (!is.null(p47_taxCO2eq_AggFE)) { # recalculating carbon prices to take into account emi Mkt taxes
      out <- out[, , setdiff(getNames(out), c(
        "Price|Carbon|Demand|Buildings (US$2017/t CO2)", "Price|Carbon|Demand|Industry (US$2017/t CO2)",
  ...
  int2ext <- .addSubvariable(int2ext, c(
    "|Fuel Cost", "|Other Taxes", "|Total LCOE",
  ...
  o01_CESderivatives <- gdx2::readGDX(gdx, "o01_CESderivatives", restoreZeros = TRUE,
                                      uniqueStyle = "classic", react = "silent")
  ...
  if (!is.null(o01_CESderivatives)) {
    ...
    ppfen_industry_dyn37 <- gdx2::readGDX(gdx, "ppfen_industry_dyn37")
    ppfen_buildings_dyn36 <- gdx2::readGDX(gdx, "ppfen_buildings_dyn36")
    ...
    # CES Markup Cost
    p37_CESMkup <- gdx2::readGDX(gdx, "p37_CESMkup") # markup in industry
    p36_CESMkup <- gdx2::readGDX(gdx, "p36_CESMkup") # markup in buildings
  ...
  # add global prices
  map <- data.frame(region = getItems(out, dim = 1), world = "GLO", stringsAsFactors = FALSE)
  ...
  glob_price <- new.magpie(getItems(out, dim = 1), getYears(out), fill = 0)
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "peur"] * 1000
  out <- mbind(out, setNames(glob_price, "PVP1|Uranium (billionDpktU)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "peoil"] * 1000
  out <- mbind(out, setNames(glob_price, "PVP1|Oil (billionDpTWyr)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "pegas"] * 1000
  out <- mbind(out, setNames(glob_price, "PVP1|Gas (billionDpTWyr)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "pecoal"] * 1000
  out <- mbind(out, setNames(glob_price, "PVP1|Coal (billionDpTWyr)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "pebiolc"] * 1000
  out <- mbind(out, setNames(glob_price, "PVP1|Biomass (billionDpTWyr)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "good"] * 1000
  out <- mbind(out, setNames(glob_price, "PVP2|Good ()"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "perm"]
  out <- mbind(out, setNames(glob_price, "PVP3|Permit ()"))

  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "peur"] / pm_pvp[, , "good"] * tdptwyr2dpgj
  out <- mbind(out, setNames(glob_price, "Price|Uranium|World Market (US$2017/GJ)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "peoil"] / pm_pvp[, , "good"] * tdptwyr2dpgj
  out <- mbind(out, setNames(glob_price, "Price|Oil|World Market (US$2017/GJ)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "pegas"] / pm_pvp[, , "good"] * tdptwyr2dpgj
  out <- mbind(out, setNames(glob_price, "Price|Gas|World Market (US$2017/GJ)"))
  for (i in getItems(out, dim = 1)) glob_price[i, , ] <- pm_pvp[, , "pecoal"] / pm_pvp[, , "good"] * tdptwyr2dpgj
