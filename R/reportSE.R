...
reportSE <- function(gdx, regionSubsetList = NULL, t = c(seq(2005, 2060, 5), seq(2070, 2110, 10), 2130, 2150)) {
  ...
  ####### read in needed data #########
  ## sets
  teChp <- gdx2::readGDX(gdx, "teChp") # technologies that produce seel as main output und sehe as secondary output
  teReNoBio <- gdx2::readGDX(gdx, "teReNoBio") # renewable technologies except for biomass
  teCCS <- gdx2::readGDX(gdx, "teCCS") # technologies with carbon capture
  teNoCCS <- gdx2::readGDX(gdx, "teNoCCS") # technologies without CCS
  teccsinje <- gdx2::readGDX(gdx, "teccsinje", react = "silent") # CCS injection technologies
  entyPe <- gdx2::readGDX(gdx, "entyPe", format = "simplest") # primary energy types (PE)
  entySe <- gdx2::readGDX(gdx, "entySe", format = "simplest") # secondary energy types
  ...
  get_prodSE <- function(inputCarrier, outputCarrier, te = c(pe2se$all_te, se2se$all_te), name = NULL, storageLossOnly = FALSE) {
    ...
    if (length(setdiff(inputCarrier, c(entyPe, entySe))) > 0) {
      warning(paste("inputCarrier ", setdiff(inputCarrier, c(entyPe, entySe)), " is not element of entyPe or entySe"))
    }

    if (length(setdiff(outputCarrier, c(entyPe, entySe))) > 0) {
      warning(paste("outputCarrier ", setdiff(outputCarrier, c(entyPe, entySe)), " is not element of entyPe or entySe"))
    }
  ...
  ## if a category has more than one subcategory, the subcategories should be
  ## reported *explicitly*.
  out <- get_prodSE(c(entyPe, entySe), entySe, name = "SE (EJ/yr)")
  ...
  if (!(is.null(v_macBase) && is.null(vm_emiMacSector))) {
    ...
    MtCH4_2_TWa <- gdx2::readGDX(gdx, "sm_MtCH4_2_TWa", react = "silent")
    ...
    out <- mbind(
      out,
      setNames(new.magpie(cells_and_regions = getItems(prodCouple, dim = 1), years = y, fill = 0), "SE|Gases|Biomass|Waste (EJ/yr)")
    )
  }
  ...
  if ("sebiochar" %in% entySe) { # for backwards compatibility, to be removed with v360 (TD)
    ...
    out <- mbind(
      out,
      setNames(new.magpie(getItems(out, dim = 1), getYears(out), fill = 0), "SE|Biochar (EJ/yr)"),
      setNames(new.magpie(getItems(out, dim = 1), getYears(out), fill = 0), "SE|Biochar|+|w/o co-product (EJ/yr)"),
      setNames(new.magpie(getItems(out, dim = 1), getYears(out), fill = 0), "SE|Biochar|+|w/ heat (EJ/yr)"),
      setNames(new.magpie(getItems(out, dim = 1), getYears(out), fill = 0), "SE|Biochar|+|w/ heat and power (EJ/yr)"),
      setNames(new.magpie(getItems(out, dim = 1), getYears(out), fill = 0), "SE|Biochar|+|w/ liquids (EJ/yr)")
    )
  }
  ...
  if (module2realisation["trade", 2] == "se_trade") {
    ...
    out <- mbind(
      out,
      setNames(
        mselect(vm_Mport - vm_Xport, all_enty = "seh2") * TWa_2_EJ,
        "SE|Hydrogen|Net Imports (EJ/yr)"
      ),
      setNames(
        mselect(vm_Mport - vm_Xport, all_enty = "seel") * TWa_2_EJ,
        "SE|Electricity|Net Imports (EJ/yr)"
      ),
      setNames(
        mselect(vm_Mport - vm_Xport, all_enty = "seliqsyn") * TWa_2_EJ,
        "SE|Liquids|Hydrogen|Net Imports (EJ/yr)"
      ),
      setNames(
        mselect(vm_Mport - vm_Xport, all_enty = "segasyn") * TWa_2_EJ,
  ...
  seh2 <- setNames(
    dimSums(mselect(v_demSeOth, all_enty = "seh2"), dim = 3),
  ...
  out <- mbind(
    ...
    setNames(
      ...
      dimSums(
        ...
        * vm_fuExtr[, , getNames(pm_fuExtrOwnCons, dim = 2)],
        dim = 3
      )
      * TWa_2_EJ,
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(vm_demFeSector, all_enty = "seel", all_enty1 = "feels", emi_sectors = "build"), dim = 3) /
        mselect(pm_eta_conv, all_te = "tdels"),
      "SE|Input|Electricity|Buildings (EJ/yr)"
    ),
    setNames(
      dimSums(mselect(vm_demFeSector, all_enty = "seel", all_enty1 = "feels", emi_sectors = "indst"), dim = 3) /
        mselect(pm_eta_conv, all_te = "tdels"),
      "SE|Input|Electricity|Industry (EJ/yr)"
    ),
    setNames(
      dimSums(mselect(vm_demFeSector, all_enty = "seel", all_enty1 = "feelt", emi_sectors = "trans"), dim = 3) /
        mselect(pm_eta_conv, all_te = "tdelt"),
      "SE|Input|Electricity|Transport (EJ/yr)"
    ),
    setNames(
      dimSums(mselect(vm_demFeSector, all_enty = "seel", all_enty1 = "feels", emi_sectors = "cdr"), dim = 3) /
        mselect(pm_eta_conv, all_te = "tdels"),
      "SE|Input|Electricity|CDR (EJ/yr)"
    ),
    setNames(
      dimSums(mselect(vm_demSe, all_enty = "seel", all_enty1 = "seh2"), dim = 3),
    ...
    setNames(
      dimSums(vm_demSe[, , "feels"] * (1 - pm_eta_conv[, , "tdels"]), dim = 3) +
        dimSums(vm_demSe[, , "feelt"] * (1 - pm_eta_conv[, , "tdelt"]), dim = 3),
      "SE|Electricity|Transmission Losses (EJ/yr)"
    ) # this variable name was used before and might still be needed by some templates
  )
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(vm_demSe, all_enty = "seh2", all_enty1 = c("seliqsyn", "segasyn")), dim = 3) * p_share_H2DomProd *
        p_shareElec_H2 / mselect(pm_eta_conv, all_te = "elh2"),
      "SE|Input|Electricity|Hydrogen|Synthetic Fuels (EJ/yr)"
    ),
    setNames(
      dimSums(mselect(vm_demSe, all_enty = "seh2", all_enty1 = c("seliqsyn")), dim = 3) * p_share_H2DomProd *
        p_shareElec_H2 / mselect(pm_eta_conv, all_te = "elh2"),
      "SE|Input|Electricity|Hydrogen|Synthetic Fuels|+|Liquids (EJ/yr)"
    ),
    setNames(
      dimSums(mselect(vm_demSe, all_enty = "seh2", all_enty1 = c("segasyn")), dim = 3) * p_share_H2DomProd *
        p_shareElec_H2 / mselect(pm_eta_conv, all_te = "elh2"),
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(vm_demSe[, , "fegas"] + vm_demSe[, , "fegat"] - vm_prodFe[, , "fegas"] - vm_prodFe[, , "fegat"], dim = 3),
      "SE|Input|Gases|T&D losses (EJ/yr)"
    ),
    setNames(
      dimSums(vm_demSe[, , "fehes"] - vm_prodFe[, , "fehes"], dim = 3),
    ...
    setNames(
      dimSums(vm_demSe[, , "feh2s"] + vm_demSe[, , "feh2t"] - vm_prodFe[, , "feh2s"] - vm_prodFe[, , "feh2t"], dim = 3),
      "SE|Input|Hydrogen|T&D losses (EJ/yr)"
    ),
    setNames(
      dimSums(vm_demSe[, , "fesos"] - vm_prodFe[, , "fesos"], dim = 3),
